...
`.github/workflows/release.yml` builds the installers on GitHub's runners and
attaches them to a draft release, so nobody downloading JammyLayer needs Node.

```bash
npm version 1.1.0      # bumps package.json and tags the commit
git push --follow-tags
```

Pushing a `v*` tag runs three jobs: a check that the tag matches the version in
`package.json` (electron-builder names the installers from `package.json`, not
from the tag, so a mismatch would ship files labelled with the previous
release), a build matrix across Windows, macOS and Linux, and a job that opens a
**draft** release with whatever the matrix produced.

Two deliberate choices in there:

- **The release job runs even if a platform failed.** Windows is the tested
  target and a broken macOS build should not hold it up. The flip side is that a
  draft can be short a file — check what is attached before publishing it.
- **electron-builder does not publish.** `"publish": null` in `package.json`
  disables it, because adding a `repository` field is otherwise enough for it to
  infer a GitHub publisher and try to upload during a tagged CI build. The
  workflow does the upload with `gh`, which needs no third-party action.

`npm ci` needs `package-lock.json` committed — the workflow fails at the install
step without it, and `actions/setup-node`'s npm cache needs it too.

Running the workflow from the Actions tab builds the installers without
releasing anything; they land as workflow artifacts. Useful for checking the
pipeline before cutting a real tag.

Nothing is code-signed. Adding certificates later means a Windows one
(`CSC_LINK` / `CSC_KEY_PASSWORD`) and an Apple Developer ID plus notarisation
(`APPLE_ID` / `APPLE_APP_SPECIFIC_PASSWORD` / `APPLE_TEAM_ID`) as repository
secrets; electron-builder picks all of those up from the environment without
